...
impl SafetyPolicy {
    ...
    pub fn is_url_allowed(&self, url: &Url) -> bool {
        self.ensure_url_allowed(url).is_ok()
    }
...
pub fn health_url_for_local_campaign(campaign_url: &Url) -> Option<Url> {
    if !is_loopback_url(campaign_url) {
        return None;
    }
    let mut health_url = campaign_url.clone();
