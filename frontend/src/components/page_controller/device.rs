...
impl Component for DevicesDrop {
    ...
    fn view(&self) -> Html {
        ...
        <div class="data-dropdown">
                <MatSelect label="Device" outlined=true icon="event" onselected=on_select>
                    <MatListItem value="0" graphic=GraphicType::Icon selected=app_route_matches(AppRoute::Devices, Rc::clone(&self.props.state))>{"By Type"}</MatListItem>
                    <MatListItem value="1" graphic=GraphicType::Icon selected=app_route_matches(AppRoute::Brand, Rc::clone(&self.props.state))>{"Brand"}</MatListItem>
                    <MatListItem value="2" graphic=GraphicType::Icon selected=app_route_matches(AppRoute::Model, Rc::clone(&self.props.state))>{"Model"}</MatListItem>
                </MatSelect>
        </div>
        }
    }
