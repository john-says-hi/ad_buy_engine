...
impl Component for OfferBtn {
        ...
        match msg {
            Msg::Click => {
                ...
                self.props
                    .state
                    .borrow_mut()
                    .set_first_prime_column_and_reset_other_columns_and_save_to_browser_and_set_route(AppRoute::Offers);
                self.router.send(ChangeRoute(AppRoute::Offers.into()))
            }
            Msg::Ignore => {}
        }
        false
    ...
    fn view(&self) -> Html {
        ...
        let icon_class = if self.active {
            "active-tab fa fa-money uk-display-block uk-text-center"
        } else {
            "fa fa-money uk-display-block uk-text-center"
        };
        html! {
        <li onclick=callback>
            <span class=icon_class></span>
            <a class=a_class>{"Offers"}</a>
        </li>
        }
    }
}
