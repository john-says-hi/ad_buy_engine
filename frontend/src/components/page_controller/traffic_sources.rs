...
impl Component for TrafficBtn {
    ...
    fn create(props: Self::Properties, link: ComponentLink<Self>) -> Self {
        ...
        let active = tab_is_active!(AppRoute::Traffic, active_tab);
    ...
    fn change(&mut self, props: Self::Properties) -> ShouldRender {
        let active_tab = state_clone!(props.state).borrow().return_app_route();
        self.active = tab_is_active!(AppRoute::Traffic, active_tab);
    ...
    fn view(&self) -> Html {
        ...
        let icon_class = if self.active{"active-tab fa fa-exchange uk-display-block uk-text-center"}else { "fa fa-exchange uk-display-block uk-text-center" };
        html! {
        <li onclick=callback>
            <span class=icon_class></span>
            <a class=a_class>{"Traffic Sources"}</a>
        </li>
        }
    }
}
