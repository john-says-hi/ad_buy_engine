...
use crate::components::page_utilities::crud_element::complex_sub_component::sequence_builder::SequenceBuilder;
...
pub struct FunnelViewRenderer {
    link: ComponentLink<Self>,
    props: Props,
    weight: String,
}

impl Component for FunnelViewRenderer {
    type Message = Msg;
    ...
    pub fn render_view(&self) -> VNode {
        ...
        match self.props.active_element {
            ActiveElement::Funnel => VNode::from(html! {
                <FunnelViewBasicData state=Rc::clone(&self.props.state) funnel_name=&self.props.funnel_name funnel_country=&self.props.funnel_country default_referrer_handling=&self.props.default_referrer_handling notes=&self.props.notes update_name=self.link.callback(Msg::UpdateName) update_country=self.link.callback(Msg::UpdateCountry) update_referrer_handling=self.link.callback(Msg::UpdateDefaultReferrerHandling) update_notes=self.link.callback(Msg::UpdateNotes) />
            }),

            ActiveElement::DefaultSequence(seq_id) => VNode::from(html! {
                <SequenceBuilder state=Rc::clone(&self.props.state) active_element=active_element conditional_sequences=conditional_sequences update_sequence=self.link.callback(Msg::UpdateSequence) update_name=self.link.callback(Msg::UpdateName) update_referrer_handling=self.link.callback(Msg::UpdateDefaultReferrerHandling) update_sequence_type=self.link.callback(Msg::UpdateSequenceType) default_sequences=&self.props.default_sequences />
            }),
