...
pub struct CustomConversionBtn {
...
impl Component for CustomConversionBtn {
        ...
        match msg {
            Msg::Click => {
                self.props
                    .state
                    .borrow_mut()
                    .set_app_route_and_save_to_browser(AppRoute::CustomConversions);
                self.router
                    .send(ChangeRoute(AppRoute::CustomConversions.into()));
            }
            Msg::Ignore => {}
        }
        false
    ...
    fn view(&self) -> Html {
        ...
        html! {
            <MatTab icon="campaign" label="Custom Conversions" stacked=true is_min_width_indicator=true oninteracted=callback />
        }
    }
}
