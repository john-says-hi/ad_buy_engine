...
use crate::storage::demo::{seed_fake_affiliate_network_catalog, seed_fake_landing_page_catalog};
...
pub async fn connect_database(config: &ServerConfig) -> ServerResult<SqlitePool> {
    ...
    if config.demo_seed_fake_landing_pages {
        seed_fake_landing_page_catalog(&pool, config).await?;
    }
    Ok(pool)
}
